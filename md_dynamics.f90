subroutine dynamics(nt,m,p,r,dvdr,dvdr2,na,nm,nb,boxlxyz,z,beta, &
  ...
  do k = 1,m !Anzahl paralleler Trajektorien für Dynamik
     ...
     ! Run an NVT trajectory of 2*nt time steps

     call trajectory(p,r,v,dvdr,dvdr2,dct,dtv,dmtr,dmtv,dqqt, &
                     davx,davy,davz,dmot1,nt,na,nb,boxlxyz,z, &
  ...
  if (itcf(3).eq.1) then
     call print_orientation(dtfs,nt,dmo1)
  endif
  write(6,*)'-------------------------------------------'
...
66 format(1x,'<PE> = ',f10.4,' Kj/mol')
  ...
  deallocate (ct,dct,rmtr,rmtv,dmtr,dmtv,dmo1,dmot1)

  return
end subroutine dynamics

subroutine trajectory(p,r,v,dvdr,dvdr2,ct,dtv,dmtr,dmtv,dqq, &
                      davx,davy,davz,dmot1,nt,na,nb,boxlxyz,z, &
                      beta,dt,mass,itcf,nm,pe,irun,dcvinter, &
                      dcvintra,dke,pt,pb,print)
  implicit none
  include 'globals.inc'
  ! -----------------------------------------------------------------
  ...
  integer i,ib,j,jt,it,it10,jt10,k,pt,pb,print(3)
