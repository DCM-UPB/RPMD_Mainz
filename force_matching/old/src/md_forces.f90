subroutine forces(r,v,dvdr,dvdr_split,nb,na,boxlxyz,z,virial,iopt)
  ...
  include 'globals.inc'
    ...
    real(8) r(3,na,nb),dvdr(3,na,nb),dvdr_split(3,na,nb,4),z(na),boxlxyz(3)
    ...
    !dvdr_split=0.d0 in potentl_opt
    ...
    if (iopt.ne.5) then
        do k = 1, nb
            ...
            call potenl_opt(r(:,:,k),dv,dvdr(:,:,k),dvdr_split(:,:,k,:),vir(:,:), &
...
subroutine potenl_opt(r,v,dvdr,dvdr_split,vir,na,nb,boxlxyz, &
  ...
  include 'globals.inc'
    ...
    logical iamcub
    common /ew_param/ alpha,alphah,ecut,wm,wh
    common /oo_param/ oo_eps,oo_sig,oo_gam,rcut
    ...
    if (iopt.le.1 .or. iopt.eq.3) then
        dvdr_split(:,:,2)=0.d0
    end if
    ...
    if (iopt.le.2) then
        ...
        dvdr_split(:,:,1)=dvdr(:,:) !save forces

        alphjac(1,:) = (/           alpha           ,       1.d0 - alphah       ,       1.d0 - alphah       /)
        alphjac(2,:) = (/   0.5d0 * (1.d0 - alpha)  ,           alphah          ,           0.d0            /)
        alphjac(3,:) = (/   0.5d0 * (1.d0 - alpha)  ,           0.d0            ,           alphah          /)

        msmat(1,:) =   (/           alpha           ,   0.5d0 * (1.d0 - alpha)  ,   0.5d0 * (1.d0 - alpha)  /)
        ...
        !store coulombic force contribution
        dvdr_split(:,:,1)=dvdr(:,:)-dvdr_split(:,:,1)
    ...
    if ((iopt.eq.0).or.(iopt.eq.1).or.(iopt.eq.3)) then
        ...
        !store LJ force contribution
        dvdr_split(:,:,2)=dvdr(:,:)-dvdr_split(:,:,2)
    endif
    ...
    if ((iopt.eq.0).or.(iopt.eq.4)) then
        ...
        if (alp.eq.0.d0) then
            write(6,*) 'intra_harmonic not implemented... Stop.'
            stop
        !!!        call intra_spcfw(r,dvdr,na,vint,vir_int)
        else
            if (alpb.ne.0.d0) then
                write(6,*) 'intra_morse not implemented... Stop.'
                stop
            else
...
subroutine full_forces(r,na,nb,v,vew,voo,vint,vir,z,boxlxyz, &
  ...
  include 'globals.inc'
    ...
    if (rpmddft.eq.0) then
        ...
        call forces(r,vint,dvdr2,dvdr_split,nb,na,boxlxyz,z,vir_itr,4)                     !dvdr2 wird in forces = 0 gesetzt
        ...
        if ((nb.eq.1).or.(nbdf1.eq.0)) then
            call forces(r,vew,dvdr,dvdr_split,nb,na,boxlxyz,z,vir_ew,2)
        else if (sig.gt.0.d0) then
     
            ! Short range coulombic correction
     
            call forces(r,ve,dvdre,dvdr_split,nb,na,boxlxyz,z,vir_ewc,5)
        ...
        if ((nb.eq.1).or.(nbdf2.eq.0)) then
            call forces(r,voo,dvdrl,dvdr_split,nb,na,boxlxyz,z,vir_oo,3)
        endif
